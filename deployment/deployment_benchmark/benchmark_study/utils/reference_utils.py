...
class AssemblyStore:
    ...
    def retrieve_local_assembly(self, passport: Passport) -> Optional[LocalAssembly]:
        ...
        if passport.accession is None:
            files = os.listdir(taxid_subdir)
            if not files:
                self.logger.warning(f"No files found in assembly directory for taxid {passport.taxid}")
                return None
        else:
            accid_subdir = f"{self.store_path}/{passport.taxid}/{passport.accession}"
            if os.path.exists(accid_subdir) is False:
                self.logger.warning(f"No assembly directory found for taxid {passport.taxid} and accession {passport.accession}")
                return None
            files = os.listdir(accid_subdir)
            if not files:
                self.logger.warning(f"No files found in assembly directory for taxid {passport.taxid} and accession {passport.accession}")
                return None
    ...
    def match_taxid_to_assembly(
        ...
        for index, row in df.iterrows():
            taxid = str(int(row['taxid'])) if taxid_col else None
            accession = str(row['accid']) if accid_col else None
